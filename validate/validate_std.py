...
#                         else:
#                             # Handle the case where v is not a Tensor (e.g., it's a list)
#                             # You can add appropriate handling or conversion here
#                             # print(v)
#                             # 1. NumPy 배열로 변환
#                             # v = np.array(v)
#                             # 2. NumPy 배열을 PyTorch Tensor로 변환
#                             # v = torch.from_numpy(v)
#                             # pred[k] = torch.rot90(v, 0, [2, 3])
#                             pass
#                 else:
#                     if isinstance(pred, torch.Tensor):
#                         pred = torch.rot90(pred, 3, [2, 3])
#                     else:
#                         # Handle the case where pred is not a Tensor (e.g., it's a list)
#                         # You can add appropriate handling or conversion here
#                         # print(pred)
#                         # 1. NumPy 배열로 변환
#                         numpy_array1 = np.array(pred)
#                         # 2. NumPy 배열을 PyTorch Tensor로 변환
#                         pred = torch.from_numpy(numpy_array1)
#                         pred = torch.rot90(pred, 3, [2, 3])
#                         # pass
...
#                     # 평균 및 스택 전에 텐서 형태인지 확인하는 부분 추가
#                     tensor_list = [p[k] for p in pred_list if isinstance(p.get(k), torch.Tensor) and p.get(k).numel() > 0]
#                     # pred[k] = torch.mean(torch.stack([p[k] for p in pred_list if isinstance(p[k], torch.Tensor)]),0)
#                     if tensor_list:
#                         pred[k] = torch.mean(torch.stack(tensor_list), 0)
#                     else:
#                         # 위 tensor_list가 비었을 때 처리
#                         pred[k] = torch.zeros(1) 
#             else:
#                 # pred = torch.mean(torch.stack([p for p in pred_list if isinstance(p, torch.Tensor)]), 0)
#                 # 평균 및 스택 전에 텐서 형태인지 확인하는 부분 추가
#                 tensor_list = [p for p in pred_list if isinstance(p, torch.Tensor) and p.numel() > 0]
#                 if tensor_list:
#                     pred = torch.mean(torch.stack(tensor_list), 0)
#                 else:
#                     # 위 tensor_list가 비었을 때 처리
#                     pred = torch.zeros(1)
